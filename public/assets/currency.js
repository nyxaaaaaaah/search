export const CURRENCIES = {
  USD: { name: "US dollar", symbol: "$", aliases: ["dollar", "dollars", "buck", "bucks", "usd", "us dollar", "american dollar"] },
  EUR: { name: "euro", symbol: "€", aliases: ["euro", "euros"] },
  GBP: { name: "British pound", symbol: "£", aliases: ["pound", "pounds", "quid", "sterling", "british pound"] },
  JPY: { name: "Japanese yen", symbol: "¥", aliases: ["yen", "japanese yen"] },
  CNY: { name: "Chinese yuan", symbol: "¥", aliases: ["yuan", "renminbi", "rmb", "chinese yuan"] },
  AUD: { name: "Australian dollar", symbol: "A$", aliases: ["aussie dollar", "australian dollar"] },
  CAD: { name: "Canadian dollar", symbol: "C$", aliases: ["loonie", "canadian dollar"] },
  ...
  NZD: { name: "New Zealand dollar", aliases: ["kiwi dollar", "new zealand dollar"] },
  INR: { name: "Indian rupee", symbol: "₹", aliases: ["rupee", "rupees", "indian rupee"] },
  KRW: { name: "South Korean won", symbol: "₩", aliases: ["won", "korean won"] },
  ...
  RUB: { name: "Russian ruble", symbol: "₽", aliases: ["ruble", "rouble", "russian ruble"] },
  ...
  ILS: { name: "Israeli shekel", symbol: "₪", aliases: ["shekel", "israeli shekel"] },
  CLP: { name: "Chilean peso", aliases: ["chilean peso"] },
  PHP: { name: "Philippine peso", aliases: ["philippine peso", "filipino peso"] },
  AED: { name: "UAE dirham", aliases: ["dirham", "uae dirham", "emirati dirham"] },
  ...
  VND: { name: "Vietnamese dong", symbol: "₫", aliases: ["dong", "vietnamese dong"] },
  NGN: { name: "Nigerian naira", symbol: "₦", aliases: ["naira", "nigerian naira"] },
  EGP: { name: "Egyptian pound", aliases: ["egyptian pound"] },
  PKR: { name: "Pakistani rupee", aliases: ["pakistani rupee"] },
  BDT: { name: "Bangladeshi taka", symbol: "৳", aliases: ["taka", "bangladeshi taka"] },
  UAH: { name: "Ukrainian hryvnia", symbol: "₴", aliases: ["hryvnia", "ukrainian hryvnia"] },
  TWD: { name: "Taiwan dollar", aliases: ["taiwan dollar", "new taiwan dollar"] },
...
export const PRIORITY = [
  ...
  "NZD", "INR", "KRW", "MXN", "BRL", "ZAR", "RUB", "TRY", "SEK", "NOK",
  "DKK", "PLN", "THB", "IDR",
];

const SYMBOLS = {
  $: "USD", "€": "EUR", "£": "GBP", "¥": "JPY", "₹": "INR", "₩": "KRW",
  "₽": "RUB", "₺": "TRY", "₪": "ILS", "฿": "THB", "₫": "VND", "₦": "NGN",
  "₴": "UAH", "৳": "BDT",
};
...
const CONNECTORS = new Set([
  "to", "in", "into", "as", "vs", "=", "->", "→", "convert", "converted",
  "conv", "exchange", "worth", "equals", "equal", "of", "is", "how", "much",
  "many", "a", "an", "the", "please", "whats", "what", "are", "for",
]);
...
const fuzzyCurrency = (tok) => {
  ...
  for (const { key, code } of fuzzyTargets) {
    ...
    if (rr < bestRank) { bestRank = rr; best = code; }
  }
  return best;
};
...
export function parseCurrencyQuery(query) {
  ...
  while (i < words.length) {
    ...
    if (two && aliasMap[two]) { found.push(aliasMap[two]); i += 2; continue; }

    const ex = exactCurrency(words[i]);
    if (ex) { found.push(ex); i++; continue; }

    if (CONNECTORS.has(words[i])) { sawConnector = true; i++; continue; }

    if (strong && fuzzyConnector(words[i])) { sawConnector = true; i++; continue; }

    const fz = strong ? fuzzyCurrency(words[i]) : null;
    if (fz) { found.push(fz); i++; continue; }
